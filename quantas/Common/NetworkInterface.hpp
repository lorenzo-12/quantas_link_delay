...
namespace quantas{
    ...
    protected:
        ...
        void                               byzantine_broadcast   (message m1, message m2, const vector<interfaceId>& group_1, const vector<interfaceId>& group_2);
    ...
    public:
        ...
        void                               setDistribution       (Distribution dist)                        {_distribution = dist;};
    ...
    template <class message>
    void NetworkInterface<message>::byzantine_broadcast(message m1, message m2, const vector<interfaceId>& group_1, const vector<interfaceId>& group_2){
    ...
    void NetworkInterface<message>::transmit(){
        ...
        while(!_outStream.empty()){
            ...
            //printf("[transmit] (%ld,%ld)  channel_delay: %d\n", outMessage.sourceId(), outMessage.targetId(), _outBoundChannelDelays.at(outMessage.targetId()));
            //cout << "[trasmit] (" << outMessage.sourceId() << "," << outMessage.targetId() << ") " << _outBoundChannelDelays.at(outMessage.targetId()) << endl;
			_outStream.pop_front();
			if (_id == outMessage.targetId()) {
                interfaceId src = outMessage.sourceId();
